...
class invalidSoundFile : public std::runtime_error {
public:
	invalidSoundFile() : runtime_error("Not a WAV or AIFF file (corrupted?)") { }
};

class compressedSoundFile : public std::runtime_error {
public:
	compressedSoundFile() : runtime_error("Compressed WAV files are not supported") { }
};

class multiTrackFile : public std::runtime_error {
public:
	multiTrackFile() : runtime_error("File neither mono nor stereo (maybe multi-track or corrupted?)") { }
};

class invalidBitDepth : public std::runtime_error {
public:
	invalidBitDepth() : runtime_error("Bit depth not 8, 16 or 24 bits") { }
};

class invalidSampRate : public std::runtime_error {
public:
	invalidSampRate() : runtime_error("Unsupported sample rate") { }
};
